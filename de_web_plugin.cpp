...
void DeRestPluginPrivate::addSensorNode(const deCONZ::Node *node, const deCONZ::NodeEvent *event)
{
    ...
    DBG_Printf(DBG_INFO, "[555] Checking for new sensors\n");
    QString modelId;
    ...
    if (node->nodeDescriptor().manufacturerCode() == VENDOR_JENNIC && modelId.isEmpty())
    {
        ...
        DBG_Printf(DBG_INFO, "[555] Trust comparable check\n");
             ...
             inClusterCount == 9 && outClusterCount == 11)
        {
            ...
            DBG_Printf(DBG_INFO, "[555] Setting ModelID and Manufacturer\n");
            //QList<deCONZ::ZclCluster>::const_iterator endc = sd.inClusters().constEnd();
            //QList<deCONZ::ZclCluster> asd = node->simpleDescriptors().inClusters().size();
            DBG_Printf(DBG_INFO, "[555] In-Clusters: %d\n", inClusterCount);
            DBG_Printf(DBG_INFO, "[555] Out-Clusters: %d\n", outClusterCount);
        }*/
    }
    ...
    for (;i != end; ++i)
    {
        ...
        {   // scan server clusters of endpoint
            ...
            for (; ci != cend; ++ci)
            {
                ...
                case BASIC_CLUSTER_ID:
                {
                    DBG_Printf(DBG_INFO, "[555] Basic cluster\n");
                ...
                {
                    DBG_Printf(DBG_INFO, "[555] power sensors\n");
                ...
                case IAS_ZONE_CLUSTER_ID:
                {
                    DBG_Printf(DBG_INFO, "[555] IAS sensors\n");
                ...
                {
                    DBG_Printf(DBG_INFO, "[555] temperature sensors\n");
                ...
                {
                    DBG_Printf(DBG_INFO, "[555] humidity sensors\n");
                ...
                {
                    DBG_Printf(DBG_INFO, "[555] Mesurement cluster\n");
                    fpPowerSensor.inClusters.push_back(ci->id());
                }
                    break;
        ...
        if (modelId.isEmpty())
        {
            DBG_Printf(DBG_INFO, "[555] Model ID still empty\n");
        ...
        DBG_Printf(DBG_INFO, "[555] Creating sensors\n");
        // ZHASwitch
        ...
        if (fpTemperatureSensor.hasInCluster(TEMPERATURE_MEASUREMENT_CLUSTER_ID))
        {
            ...
            if (!sensor || sensor->deletedState() != Sensor::StateNormal)
            {
                ...
                DBG_Printf(DBG_INFO, "[555] Creating temperature sensors\n");
            }
            else
            {
                checkSensorNodeReachable(sensor);
        ...
        if (fpHumiditySensor.hasInCluster(RELATIVE_HUMIDITY_CLUSTER_ID))
        {
            ...
            if (!sensor || sensor->deletedState() != Sensor::StateNormal)
            {
                ...
                DBG_Printf(DBG_INFO, "[555] Creating humidity sensors\n");
            }
            else
            {
                checkSensorNodeReachable(sensor);
...
void DeRestPluginPrivate::addSensorNode(const deCONZ::Node *node, const SensorFingerprint &fingerPrint, const QString &type, const QString &modelId, const QString &manufacturer)
{
    ...
    DBG_Printf(DBG_INFO, "[666] Adding API items\n");
    ...
    else if (modelId.startsWith(QLatin1String("113D")))
    {
        sensorNode.setManufacturer("iHorn");
    }
	...
	DBG_Printf(DBG_INFO, "[666] Generating UID\n");
	...
	DBG_Printf(DBG_INFO, "[666] Saving sensor data\n");
    sensorNode.setNeedSaveDatabase(true);
...
void DeRestPluginPrivate::handleZclAttributeReportIndicationXiaomiSpecial(const deCONZ::ApsDataIndication &ind, deCONZ::ZclFrame &zclFrame)
{
    ...
    while (!stream.atEnd())
    {
        ...
        {
            DBG_Printf(DBG_INFO, "\t9b device LED on/off %d\n", u8);
        }
        else if (tag)
        {
...

    {
        ...
        // simple descriptor for endpoint 0x01
        {
            ...
            for (size_t i = 0; i < node->endpoints().size(); i++)
            {
                ...
                DBG_Printf(DBG_INFO, "[000] simple descriptor - EP 0x%02X\n", ep);

                for (int j = 0; j < node->simpleDescriptors().size(); j++)
                {
                    DBG_Printf(DBG_INFO, "[000] simple descriptor - SD 0x%02X\n", j);
                    const deCONZ::SimpleDescriptor &sd = node->simpleDescriptors()[j];
                    if (sd.endpoint() == ep && sd.deviceId() != 0xffff)
                    {
                        DBG_Printf(DBG_INFO, "[000] simple descriptor - BREAK\n");
                        ep = 0;
                        break;
                    }
                }
        ...
        if (!sensor || modelId.isEmpty() || manufacturer.isEmpty() || (swBuildId.isEmpty() && dateCode.isEmpty() && (dateCodeAvailable || swBuildIdAvailable)))
        {
            ...
            {
                DBG_Printf(DBG_INFO, "[444] return 1 (modelId empty or device not supported)\n");
                return;
            }

            if (basicClusterEndpoint == 0)
            {
                DBG_Printf(DBG_INFO, "[444] return 2 (basic cluster enpoint equals 0)\n");
                return;
            }
            ...
            if (skip)
            {
                DBG_Printf(DBG_INFO, "[444] skip true (Xiaomi stuff)\n");
                // don't read these (Xiaomi, Trust, ...)
                // response is empty or no response at all
            }
            if (modelId.isEmpty()) { attributes.push_back(0x0005); DBG_Printf(DBG_INFO, "[444] Model ID empty\n"); } // model id
            if (manufacturer.isEmpty()) { attributes.push_back(0x0004); DBG_Printf(DBG_INFO, "[444] Manufacturer empty\n"); }// manufacturer
            ...
            else if (swBuildId.isEmpty() && dateCode.isEmpty())
            {
                    ...
                    !swBuildIdAvailable)
                {
                    ...
                    DBG_Printf(DBG_INFO, "[444] date code empty\n");
                }
                else
                {
                    attributes.push_back(0x4000); // sw build id
                    DBG_Printf(DBG_INFO, "[444] SW build empty\n");
                }
            }
            ...
            QByteArray ba1 = manufacturer.toLocal8Bit();
            const char *manu = ba1.data();
            
            DBG_Printf(DBG_INFO, "[444] Model ID length: %d\n", modelId.length());
            DBG_Printf(DBG_INFO, "[444] Model ID: %s\n", mid);
            DBG_Printf(DBG_INFO, "[444] Manufacturer length: %d\n", manufacturer.length());
            ...
            { // filter for available basic cluster attributes
                ...
                for (auto id: tmp)
                {
                    ...
                    {
                        DBG_Printf(DBG_INFO, "[444] unsupp pushback\n");
            ...
            {
                DBG_Printf(DBG_INFO, "[444] No sensor, let's add it?\n");
                addSensorNode(node);
            }
